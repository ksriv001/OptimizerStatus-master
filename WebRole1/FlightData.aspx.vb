...
Public Class FlightData
    ...
    Protected Sub LinkSearch_Click(sender As Object, e As EventArgs) Handles LinkSearch.Click
        ...
        If DrpCarrier.SelectedValue <> 100 And DrpCarrier.SelectedValue <> 0 Then
            req1 = "SELECT CAST(f.[DepartureDateGMT] + ' ' + [DepartureTimeGMT] as datetime) Dep,CAST(f.[ArrivalDateGMT] + ' ' + [ArrivalTimeGMT] as datetime) arr,t.RequesterName,fnl.Operator,fnl.Registration,fnl.BrokerAircraft,fnl.HomeBaseAirportCode,fnl.BaseCode,cast(ltrim(rtrim(t.quotedtotalcost)) as money)/100 as quotedtotalcost,quotedOwnerACcost,f.tripnumber, f.AircraftID,  f.aircrafttype, "
        Else
            req1 = "SELECT CAST(f.[DepartureDateGMT] + ' ' + [DepartureTimeGMT] as datetime) Dep,CAST(f.[ArrivalDateGMT] + ' ' + [ArrivalTimeGMT] as datetime) arr,t.RequesterName,'' as Operator,'' as Registration,'' as BrokerAircraft,'' as HomeBaseAirportCode,'' as BaseCode,cast(ltrim(rtrim(t.quotedtotalcost)) as money)/100 as quotedtotalcost,quotedOwnerACcost,f.tripnumber, f.AircraftID,  f.aircrafttype, "
        End If
        '20161023 - pab - use icao because iata not always populated in fosflights
        ...
        req1 &= "duration, deadhead, paxcount, legtypecode, legratecode, legpurposecode, tripstatus, legstate, proratedrevenue, PIC, SIC, w.AircraftWeightClass  FROM [FOSFlights] f  WITH (NOLOCK)     "
        ...
        '20160329 - pab - add requester and total cost
        req1 &= "LEFT JOIN fostrips t ON f.carrierid=t.carrierid and f.tripnumber=t.tripnumber LEFT JOIN AircraftWeightClass w ON f.AircraftType=w.AircraftType  "

        '20170623 - kan - add join for Registration field(not for WU)
        If DrpCarrier.SelectedValue <> 100 And DrpCarrier.SelectedValue <> 0 Then
            req1 &= "LEFT JOIN (SELECT * from AIRCRAFT c RIGHT JOIN (select max(casupdate) cmx,FOSAircraftID ss from Aircraft  GROUP BY fosaircraftid )k on k.cmx=c.CASUpdate and c.FOSAircraftID=k.ss ) fnl on f.AircraftID=fnl.ss "
        End If
        ...
        If txtAircraftID.Text <> "" Then
            req = req & "and f.aircraftid LIKE '%pqr234%' "
            req = Replace(req, "pqr234", txtAircraftID.Text)
        End If
        ...
        If Drpactype.SelectedValue <> "All" Then
            ...
            Select Case Drpactype.SelectedValue
                Case "H80H"
                    req = req & " and (f.aircrafttype = 'H80H' or f.aircrafttype = 'H850') "

                Case "B350F", "B350I"
                    req = req & " and (f.aircrafttype = 'B350F' or f.aircrafttype = 'B350I') "

                Case Else
                    req = req & " and f.aircrafttype = 'abc123' "
        ...
        If CHKstale1.Checked = True Then
            req = req & " and t.casupdate > DateAdd(hh, -1, SYSUTCDATETIME()) "
            req = req & " and f.casupdate > DateAdd(hh, -1, SYSUTCDATETIME()) "
        End If
        ...
        If DrFlightType.SelectedValue <> "All" Then
            ...
            If DrFlightType.SelectedValue = "R" Then
                ...
                If DrpCarrier.SelectedValue = "100" Then
                    req = req & " and legtypecode in ('CONT','R135','R','SHTL','TECH','HOT') and deadhead = 'False'   "
                    'Revenue logic for Delta
                ElseIf DrpCarrier.SelectedValue = "108" Then
                    req = req & " and (legtypecode in ('CHRE','CHWH','CHTR','181','COMP','179','JLDS') or legtypecode like 'JF%' ) and deadhead = 'False'  "
                Else
            ...
            End If
            If DrFlightType.SelectedValue = "S" Then
                req = req & " and legtypecode = 'SWAP' and  f.arrivalairporticao=f.departureairporticao "
            End If
            If DrFlightType.SelectedValue = "MX" Then
                req = req & " and legtypecode = 'MXSC' and  f.arrivalairporticao=f.departureairporticao "
            End If
            If DrFlightType.SelectedValue = "N" Then
                req = req & " and legtypecode = 'NC' and  f.arrivalairporticao=f.departureairporticao "
            End If
            If DrFlightType.SelectedValue = "A" Then
                req = req & " and legtypecode = 'AOG' and  f.arrivalairporticao=f.departureairporticao "
            End If
            If DrFlightType.SelectedValue = "AV" Then
                req = req & " and legtypecode = 'AVAI' and  f.arrivalairporticao=f.departureairporticao "
            End If
            If DrFlightType.SelectedValue = "OCF" Then
                req = req & " and legtypecode = 'OCF' and  f.arrivalairporticao=f.departureairporticao "
            End If
            If DrFlightType.SelectedValue = "SICK" Then
                req = req & " and legtypecode = 'SICK' and  f.arrivalairporticao=f.departureairporticao "
            End If
            If DrFlightType.SelectedValue = "Y" Then
                req = req & " and legtypecode = 'Y' and  f.arrivalairporticao=f.departureairporticao "
            End If
        ...
        If DrpCarrier.SelectedValue = 108 Then
            If DrpBkr.SelectedValue = "YESBKR" Then
                req = req & "and BrokerAircraft = 'TRUE' and (PIC = '' and SIC = '') "
            ElseIf DrpBkr.SelectedValue = "NOBKR" Then
                req = req & "and (PIC <> '' or SIC <> '')"
            End If
        Else
            If DrpBkr.SelectedValue = "YESBKR" Then
                req = req & "and (PIC = '' and SIC = '')"
            ElseIf DrpBkr.SelectedValue = "NOBKR" Then
                req = req & "and (PIC <> '' or SIC <> '')"
            End If
        End If

        'If chkOneWayTrip.Checked Then
        '    req = req & "and quotedtotalcost=proratedrevenue"
        'End If



        If chkRoundtrip.Checked Then
            req = req & "and f.tripnumber IN
                            (
                                select tripnumber from 
                                    (
                                        select q1.* from 
                                            (" & req1 & req & ") q1 JOIN (" & req1 & req & ") q2  ON q1.tripnumber=q2.tripnumber 
                                               and q1.DepartureAirportICAO=q2.ArrivalAirportICAO and q1.ArrivalAirportICAO=q2.DepartureAirportICAO  
                                               where datediff(second,q1.dep,q2.arr)/3600.0 between 0.0 and 24.0 or datediff(second,q2.dep,q1.arr)/3600.0 between 0.0 and 24.0
                                            ) g
                                            group by tripnumber
                                            having count(*)=2
                                    )"
        ...
        If txtBasecode.Text <> "" Then
            req = req & "and fnl.basecode = 'thebasecode'"
            req = Replace(req, "thebasecode", txtBasecode.Text)
        ...
        Dim req2 As String
        '20160329 - pab - add requester and total cost
        req2 = "select count(*) as cnt, sum (CAST(nauticalmiles as numeric(20,2) )) as nauticalmiles,  sum (CAST(duration as numeric(20,2) )) as duration, sum(ProRatedRevenue) as prev from [FOSFlights] f WITH (NOLOCK) LEFT JOIN fostrips t ON f.carrierid=t.carrierid and f.tripnumber=t.tripnumber LEFT JOIN AircraftWeightClass w ON f.AircraftType=w.AircraftType LEFT JOIN (SELECT * from AIRCRAFT c RIGHT JOIN (select max(casupdate) cmx,FOSAircraftID ss from Aircraft GROUP BY fosaircraftid )k 
on k.cmx=c.CASUpdate and c.FOSAircraftID=k.ss ) fnl on f.AircraftID=fnl.ss  where     " & req
        lblsql.Text = kkk
        ...
        If Not (IsDBNull(rs.Fields("nauticalmiles").Value)) Then
            ...
            txtNM.Text = FormatNumber(rs.Fields("nauticalmiles").Value, 0)
        End If
